...
#include "color/desaturate.glsl" // example include from a library like lygia.
...
void main() {
  ...
  gl_FragColor = vec4(abs(sin(desaturate(color, 0.85))), 1.0); // "output", what the frag color should be
}
